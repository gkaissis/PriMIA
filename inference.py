...
if __name__ == "__main__":
    try:
        ...
        parser.add_argument(
            "--encrypted_inference",
            action="store_true",
            help="Perform encrypted inference",
        )
        parser.add_argument(
            "--websockets_config",
            default=None,
        ...
        parser.add_argument(
            "--cuda", action="store_true", help="Use CUDA acceleration."
        )
        parser.add_argument(
        ...
        if cmd_args.encrypted_inference or cmd_args.websockets_config:
            ...
            if cmd_args.websockets_config:
                ...
                assert (
                    "data_owner" in worker_names
                ), "No data_owner in websockets config"
        ...
        if not cmd_args.websockets_config:
            ...
            if cmd_args.encrypted_inference:
                data = []
                for d in tqdm(
                    dataset, total=len(dataset), leave=False, desc="load data"
                ):
                    data.append(d)
        ...
        tock = time.time()
        print()
        print(f"Took {tock-tick} seconds.")
