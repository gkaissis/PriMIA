...
class ObjectPointer(AbstractSendable):
    ...
    def handle_func_command(cls, command):
        ...
        # Send the command
        response = owner.send_command(
            location, cmd_name=cmd, args_=args_, kwargs_=kwargs_
        )
    ...
    def get(self, user=None, reason: str = "", deregister_ptr: bool = True):
        ...
        if self.location == self.owner:
            ...
            obj = self.owner.request_obj(
                self.id_at_location, self.location, user, reason
            )
...
register_forward_func(
    {ObjectPointer: lambda p: (_ for _ in ()).throw(RemoteObjectFoundError(p))}
)
