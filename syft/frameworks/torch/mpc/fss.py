...
def fss_op(x1, x2, op="eq"):
    ...
    try:
        shares = []
        for i, location in enumerate(locations):
            share = remote(mask_builder, location=location)(
                *workers_args[i], return_value=True
            )
            shares.append(share)
    ...
    if not asynchronous:
        shares = []
        for i, location in enumerate(locations):
            share = remote(evaluate, location=location)(
                *workers_args[i], return_value=False
            )
            shares.append(share)
    else:
        shares = asyncio.run(
            sy.local_worker.async_dispatch(
                workers=locations,
                commands=[
                    (full_name(evaluate), None, workers_args[i], {}) for i in [0, 1]
                ],
            )
        )
...
def mask_builder(x1, x2, op):
    ...
    alpha, s_0, *CW = worker.crypto_store.get_keys(
        f"fss_{op}", n_instances=numel, remove=False
    )
...
class DIF:
    ...
    def keygen(n_values=1):
        ...
        for i in range(0, n):
            ...
            h1 = H(s[i, 1], 1)
            # Re-use useless randomness
            σL_0, _, sL_0, _, σR_0, _, sR_0, _ = split(
                h0, (COMP, λs, 1, λs, 1, λs, 1, λs, 1)
            )
            σL_1, _, sL_1, _, σR_1, _, sR_1, _ = split(
                h1, (COMP, λs, 1, λs, 1, λs, 1, λs, 1)
            )
            s_rand = (sL_0 ^ sL_1) * α[i] + (sR_0 ^ sR_1) * (1 - α[i])
...
def split_last_bit(buffer):
    ...
    return (
        buffer & 0b1111111111111111111111111111111111111111111111111111111111111110,
        buffer & 0b1,
    )
