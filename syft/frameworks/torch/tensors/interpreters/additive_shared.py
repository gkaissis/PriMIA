...
class AdditiveSharingTensor(AbstractTensor):
    ...
    def clone(self):
        ...
        cloned_tensor.child = {
            location: share.clone() for location, share in self.child.items()
        }
    ...
    def generate_shares(self, secret, n_workers, random_type):
        ...
        """
        random_type = (
            torch.LongTensor if random_type == torch.int64 else torch.IntTensor
        )
            ...
            for index in indices_shares:
                if isinstance(index, slice):
                    ...
                    raise NotImplementedError(
                        "Index type", type(indices), "not supported"
                    )
    ...
    def _public_mul(self, shares, other, equation):
        ...
        if isinstance(other, dict):
            return {
                worker: (self.modulo(cmd(share, other[worker])))
                for worker, share in shares.items()
            }
        else:
            return {
                worker: (self.modulo(cmd(share, other)))
                for worker, share in shares.items()
            }
        ...
        def functional(module):
            ...
            def split(tensor_shares, *args, **kwargs):
                ...
                for worker, share in tensor_shares.items():
                    ...
                    if results is None:
                        results = [
                            {worker: share_result} for share_result in share_results
                        ]
                    else:
                        for result, share_result in zip(results, share_results):
                            result[worker] = share_result
            ...
            def functional(module):
                ...
                def pad(input_shares, pad, mode="constant", value=0):
                    ...
                    for location, shares in input_shares.items():
                        padded_shares[location] = torch.nn.functional.pad(
                            shares, pad, mode, value
                        )
    ...
    def _one_hot_to_index(self, dim, keepdim):
        ...
        if dim is None:
            ...
            result = self * torch.tensor(
                list(range(n_elem)), dtype=self.torch_dtype
            ).view(size)
    ...
    def argmax(self, dim=None, keepdim=False, one_hot=False):
        ...
        for worker, share in x.child.items():
            share = remote(helper_argmax_pairwise, location=worker)(
                share, dim, return_value=False
            )
        ...
        result = (
            ...
            if dim is None and len(self.shape) > 1
            else result
        )
        ...
        if hasattr(tensor, "child"):
            for key, value in tensor.child.items():
                sy.serde.protobuf.proto.set_protobuf_id(
                    protobuf_tensor.location_ids.add(), key
                )
    ...
    def unbufferize(
        ...
        if protobuf_tensor.location_ids is not None:
            chain = {}
            for pb_location_id, share in zip(
                protobuf_tensor.location_ids, protobuf_tensor.shares
            ):
